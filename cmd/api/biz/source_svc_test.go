...
func newFakeSourceRepo() *fakeSourceRepo {
	return &fakeSourceRepo{sources: map[string]*domain.SourceConnection{}, credentials: map[string]*domain.CredentialRef{}, secrets: map[string]string{}}
}
...
func TestSourceSvc_CreateSource_HappyPath(t *testing.T) {
	...
	input := domain.CreateSourceInput{SourceType: domain.SourceTypeGitHub, DisplayName: "acme GitHub", Config: map[string]string{"org": "acme"}, RawSecret: "ghp_thisisatesttoken1234"}
	...
	assert.NotEmpty(t, sc.CredentialID)
...
func TestSourceSvc_CreateSource_SecretNotInSource(t *testing.T) {
	svc, _ := newTestSvc(t)
	input := domain.CreateSourceInput{SourceType: domain.SourceTypeJira, DisplayName: "acme Jira", Config: map[string]string{"project": "ACME"}, RawSecret: "jira-secret-token"}
...
func TestSourceSvc_TestConnection_HappyPath(t *testing.T) {
	svc, _ := newTestSvc(t)
	input := domain.CreateSourceInput{SourceType: domain.SourceTypeGitHub, DisplayName: "source", Config: map[string]string{}, RawSecret: "ghp_testtoken1234"}
	sc, _, err := svc.CreateSource(context.Background(), "ws_01", input)
	require.NoError(t, err)
...
func TestSourceSvc_InvalidKeyLength(t *testing.T) {
	_, err := biz.NewSourceSvc(newFakeSourceRepo(), []byte("tooshort"))
	assert.Error(t, err)
}
func TestSourceSvc_Hint_ShortSecret(t *testing.T) {
	svc, repo := newTestSvc(t)
	_, cred, err := svc.CreateSource(context.Background(), "ws_01", domain.CreateSourceInput{SourceType: domain.SourceTypeGitHub, DisplayName: "short secret", Config: map[string]string{}, RawSecret: "abc"})
	require.NoError(t, err)
