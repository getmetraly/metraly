...
type DataQualityContract struct {
	...
	// CoveragePercent is the percentage of returned time buckets that have a
	// computable (non-null) value (0–100). A bucket with a null value means the
	// underlying query produced no data for that time period.
	// This is NOT a measure of expected vs actual events; it measures bucket fill.
	CoveragePercent float64 `json:"coveragePercent"`
	// EarliestDataAt is the start timestamp of the earliest non-empty bucket in the result.
	// It reflects bucket boundaries, not raw event timestamps.
	EarliestDataAt *time.Time `json:"earliestDataAt,omitempty"`
	// LatestDataAt is the start timestamp of the latest non-empty bucket in the result.
	// It reflects bucket boundaries, not raw event timestamps.
